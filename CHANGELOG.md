...
- **Принцип «один владелец на метрику»** — устранены дубли отчетности:
  - мониторинг корневой ФС выведен из модуля system (единственный
    владелец всех файловых систем — модуль disk);
  - температуры дисков (чипы drivetemp) исключены из модуля
    temperature (владелец температур дисков — модуль smart).

### Added

- **Редизайн `lsm report`**: визуальные индикаторы заполненности
  (CPU/память/swap/файловые системы), ровная таблица ФС с
  гуманизированными размерами, компактные топ-5 процессов,
  защита от сбоев парсинга `df` при пробелах в точках монтирования;
  бары заполнения в отчете модуля disk. Все строки отчета ≤ 80 символов.
